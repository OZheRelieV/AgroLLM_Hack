...
poetry run streamlit run app/streamlit_app.py > start.bat.log 2>&1
...
if exist "%VENV_PATH%\Scripts\python.exe" (
    ...
    "%PYTHON_PATH%" -m streamlit run app/streamlit_app.py >> start.bat.log 2>&1
    if %errorlevel% equ 0 goto end
)
...
where python >nul 2>nul && set "PYTHON_PATH=python" && echo Использую системный python && %PYTHON_PATH% -m streamlit run app/streamlit_app.py >> start.bat.log 2>&1
if %errorlevel% equ 0 goto end
