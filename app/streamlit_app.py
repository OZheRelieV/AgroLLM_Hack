...
with tabs[0]:
    ...
    manual_date = st.date_input("Дата отчёта", value=default_manual_date, key="manual_date")
    if st.button("Запустить анализ", type="primary"):
        try:
            ...
            with st.spinner(f"Парсинг и анализ WhatsApp за {manual_date.strftime('%d.%m.%Y')}..."):
                parser_cmd = [
                    "python", str(base_dir / "whatsapp_parser" / "wa_parser.py"), dt_str
                ]
                result1 = subprocess.run(parser_cmd, capture_output=True, text=True)
                if result1.returncode != 0:
                    st.error("Ошибка при парсинге сообщений WhatsApp. Проверьте настройки и повторите попытку.")
                elif not chat_json.exists():
                    st.warning("Нет сообщений за выбранную дату. Файл не создан.")
                else:
                    st.success(f"Сообщения успешно собраны! Использован файл: {chat_json.name}\nЗапускается анализ...")
                    model_local = "yandex_gpt_extraction"
                    script = base_dir / "llm_extraction" / f"{model_local}.py"
                    analyzer_cmd = [
                        "python", str(script), "--input", str(chat_json), "--output", str(dated_excel)
                    ]
                    result2 = subprocess.run(analyzer_cmd, capture_output=True, text=True)
                    if result2.returncode != 0:
                        st.error("Ошибка при анализе сообщений и формировании Excel-файла. Проверьте корректность данных.")
                    elif not dated_excel.exists():
                        st.warning("Excel-файл не был создан после анализа. Проверьте корректность исходных данных.")
                    else:
                        st.success(f"Excel-файл готов: {date_str}.xlsx")
                        with open(dated_excel, "rb") as f:
                            st.download_button(f"Скачать {date_str}.xlsx", f, file_name=f"{date_str}.xlsx", type="primary")
                        try:
                            df = pd.read_excel(dated_excel)
                            st.dataframe(df, use_container_width=True, hide_index=True)
                        except Exception as e:
                            st.warning("Excel-файл создан, но не удалось отобразить его содержимое.")
...
with tabs[1]:
    ...
    if platform.system() != "Windows":
        ...
        auto_time = st.time_input("Время автозапуска (часы:минуты)", value=time(23, 59), key="auto_time", step=60)
        ...
        st.markdown(f"Скрипт для ручного запуска: <code>{bat_path}</code>", unsafe_allow_html=True)
        if enable:
            ...
            if result.returncode == 0:
                st.success("Автозапуск успешно настроен! Задача появится в планировщике Windows.")
            else:
        ...
        if disable:
            ...
            result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
            if result.returncode == 0:
                st.success("Автозапуск отключён. Задача удалена из планировщика Windows.")
            else:
    ...
    if st.checkbox('Показать графики по подразделениям и операциям', value=False):
        try:
            ...
            if excel_files:
                ...
                df = df.dropna(subset=["Подразделение", "Операция", "Культура", "За день (га)"])
                df = df[df["За день (га)"].astype(str).str.strip() != ""]
                for podrazdelenie in df["Подразделение"].unique():
                    st.markdown(f"<h4 style='color:#388e3c;'>{podrazdelenie}</h4>", unsafe_allow_html=True)
                    df_podr = df[df["Подразделение"] == podrazdelenie]
                    for oper in df_podr["Операция"].unique():
                        df_oper = df_podr[df_podr["Операция"] == oper]
                        data = df_oper.groupby("Культура")["За день (га)"].apply(lambda x: pd.to_numeric(x, errors='coerce').sum()).reset_index()
                        fig = px.bar(data, x="Культура", y="За день (га)", title=f"{oper}", labels={"За день (га)": "Гектаров за день"}, color="Культура")
