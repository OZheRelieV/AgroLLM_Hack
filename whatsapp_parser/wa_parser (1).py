...
data_fir_path = script_dir.parent / "chrome_profile"
...
def get_datetime_input(prompt):
    ...
    while True:
        ...
        try:
            ...
            print(
                "Неверный формат. Пожалуйста, используйте формат ГГГГ-ММ-ДД ЧЧ:ММ или просто Enter."
            )
...
if len(sys.argv) > 1:
    try:
        ...
        print(
            f"Ошибка разбора даты из аргумента: {e}. Будет использован интерактивный ввод."
        )
...
options.add_argument(f"--user-data-dir={data_fir_path}")  # Замените путь
...
try:
    ...
    print(
        "Не удалось дождаться загрузки WhatsApp Web. Проверьте авторизацию и интернет."
    )
    driver.quit()
    exit()
...
try:
    ...
    print(
        "Возможные причины: неверное название чата, медленное интернет-соединение, изменения в интерфейсе WhatsApp."
    )
    driver.quit()
    exit()
except Exception as e:
...
try:
    ...
    print(
        f"Прокрутка вверх ({scroll_attempts} раз) для загрузки истории. Это может занять время..."
    )
    ...
    for i, msg in enumerate(message_elements):
        try:
            ...
            timestamp_attr = msg.find_element(
                By.XPATH, ".//div[contains(@class, 'copyable-text')]"
            ).get_attribute("data-pre-plain-text")

            if (
                not timestamp_attr
            ):  # Пропускаем элементы без этой информации (например, системные сообщения "Сообщение удалено")
                continue
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"time data '{dt_part}' does not match known formats"
                        )

                # --- Если дата успешно распознана одним из способов ---
                time_sent_str = msg_datetime.strftime(
                    "%H:%M:%S"
                ...
                print(
                    f"Предупреждение: Не удалось распарсить дату/время/автора из '{timestamp_attr}'. Пропуск сообщения. Ошибка: {dt_parse_error}"
                )
                continue
            ...
            if msg_datetime < start_datetime:
                ...
                try:
                    ...
                    if not text:
                        caption_element = msg.find_elements(
                            By.XPATH, ".//span[@data-testid='caption']"
                        )
                        if caption_element:
                    ...
                    print(
                        "Предупреждение: Элемент сообщения устарел во время извлечения текста. Пропуск."
                    )
                    continue
                except Exception as text_exc:
                    print(
                        f"Предупреждение: Не удалось извлечь текст сообщения. Ошибка: {text_exc}"
                    )
            ...
            print(
                f"Предупреждение: Элемент сообщения {i+1} устарел (StaleElementReferenceException). Пропуск."
            )
            ...
            print(
                f"Предупреждение: Не найден ожидаемый элемент в структуре сообщения {i+1}. Возможно, это не стандартное сообщение. Ошибка: {e}. Пропуск."
            )
            continue
        except Exception as e:
    ...
    print(
        f"Анализ завершен. Обработано сообщений: {processed_count}. Добавлено в файл: {added_count}."
    )
    ...
    if messages:
        ...
        result = [
            {"author": msg["author"], "date": msg["date"], "text": msg["text"]}
            for msg in messages
        ]
        ...
        print(
            "В указанном диапазоне дат/времени сообщений не найдено или не удалось их обработать."
        )

except TimeoutException as e:
    print(f"Ошибка ожидания элемента: {e}")
    print(
        "Возможно, страница не загрузилась полностью или структура WhatsApp Web изменилась."
    )
except Exception as e:
